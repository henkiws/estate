...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-4" id="address-history-card">
    ...
    <div class="p-6">
        <div class="flex items-start justify-between">
            ...
            <div class="flex items-start gap-4 flex-1">
                ...
                <!-- Content -->
                <div class="flex-1">
                    <h3 class="text-lg font-semibold text-gray-900">Address History</h3>
                    <p class="text-sm text-gray-500 mt-1" id="address-history-summary">
                    ...
                    <!-- Status Badge -->
                    <div class="mt-3">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $user->addresses && $user->addresses->count() > 0 ? 'bg-teal-50 text-teal-700 border border-teal-200' : 'bg-gray-50 text-gray-700 border border-gray-200' }}" id="address-history-status">
                            @if($user->addresses && $user->addresses->count() > 0)
                                Complete
                            @else
                                Incomplete
            ...
            <div class="flex items-start gap-4 ml-4">
                ...
                <div class="flex items-center justify-center w-14 h-14 rounded-full border-4 {{ $user->addresses && $user->addresses->count() > 0 ? 'border-teal-500' : 'border-gray-300' }} bg-white">
                    <span class="text-sm font-bold {{ $user->addresses && $user->addresses->count() > 0 ? 'text-teal-600' : 'text-gray-400' }}" id="address-history-percentage">
                        @if($user->addresses && $user->addresses->count() > 0)
                            100%
                        @else
                            0%
        ...
        <form method="POST" action="{{ route('user.profile.update-step') }}" class="p-6 space-y-6">
            ...
            <div class="bg-white rounded-lg p-6 space-y-4">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h4 class="text-base font-semibold text-gray-900">Address History</h4>
                        <p class="text-sm text-gray-500 mt-1">Provide details of your residential history for the past 3 years</p>
                    </div>
    ...
    const newAddressHtml = `
        <div class="address-item p-4 border-2 border-gray-200 rounded-lg mb-4" data-index="${addressIndex}">
            <div class="flex items-center justify-between mb-4">
                ...
                <button type="button" onclick="removeAddressItem(${addressIndex})" class="text-red-600 hover:text-red-700 text-sm font-medium">
                    Remove
                </button>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="addresses[${addressIndex}][reason_for_leaving]" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500" placeholder="e.g., End of lease, relocated for work, purchased property..."></textarea>
            </div>
            ...
            <div class="postal-address-field hidden" data-index="${addressIndex}">
                ...
                <input type="text" name="addresses[${addressIndex}][postal_code]" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500" placeholder="PO Box 123, Sydney NSW 2000">
            </div>
        </div>
    `;
...
function removeAddressItem(index) {
    ...
    if (item) {
        ...
        document.querySelectorAll('.address-item').forEach((el, idx) => {
            ...
            if (idx === 0) {
                heading.innerHTML = 'Address 1 <span class="px-2 py-1 bg-teal-100 text-teal-700 text-xs font-semibold rounded ml-2">Current</span>';
            } else {
