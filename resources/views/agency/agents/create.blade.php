...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="space-y-4">
        ...
        <nav class="flex items-center gap-2 text-sm">
            ...
            <span class="text-plyform-dark font-medium">Add New Agent</span>
        </nav>
        ...
        <div class="flex items-start justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-plyform-dark">Add New Agent</h1>
            ...
            <div class="hidden lg:block">
                <div class="bg-plyform-yellow/20 border-2 border-plyform-yellow rounded-xl p-4 max-w-xs">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-plyform-dark">Quick Tip</h3>
                            <p class="text-xs text-gray-700 mt-1">Enable "Send invitation" to let the agent set their own password and access their account.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <form action="{{ route('agency.agents.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="p-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" 
                                   ...
                                   class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-plyform-yellow/20 focus:border-plyform-yellow outline-none transition-all @error('phone') border-red-500 @enderror">
                        </div>
                        @error('phone')
                            <p class="mt-1.5 text-sm text-red-600 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <label for="photo-upload" 
                                ...
                                <span class="text-sm text-gray-600 group-hover:text-plyform-dark" id="file-name">Choose photo...</span>
                            </label>
                        </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="bio" 
                              ...
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-plyform-yellow/20 focus:border-plyform-yellow outline-none transition-all resize-none @error('bio') border-red-500 @enderror">{{ old('bio') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        @foreach(['Residential Sales', 'Commercial Sales', 'Property Management', 'Leasing', 'Auctions', 'Luxury Properties', 'First Home Buyers', 'Investments'] as $spec)
                            <label class="relative flex items-center gap-3 p-3.5 border-2 border-gray-200 rounded-xl hover:border-plyform-purple/50 hover:bg-plyform-purple/5 cursor-pointer transition-all group">
                                ...
                                <span class="text-sm font-medium text-gray-700 group-hover:text-plyform-purple">{{ $spec }}</span>
                        ...
                        @foreach(['English', 'Mandarin', 'Cantonese', 'Arabic', 'Vietnamese', 'Spanish', 'Italian', 'Greek'] as $lang)
                            <label class="relative flex items-center gap-3 p-3.5 border-2 border-gray-200 rounded-xl hover:border-plyform-purple/50 hover:bg-plyform-purple/5 cursor-pointer transition-all group">
                                <input type="checkbox" 
                                       ...
                                       value="{{ $lang }}"
                                       class="w-4 h-4 text-plyform-purple border-gray-300 rounded focus:ring-plyform-purple/20">
                                <span class="text-sm font-medium text-gray-700 group-hover:text-plyform-purple">{{ $lang }}</span>
                                <div class="absolute inset-0 rounded-xl ring-2 ring-plyform-purple opacity-0 group-has-[:checked]:opacity-100 pointer-events-none"></div>
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select name="state" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-plyform-mint/30 focus:border-plyform-mint outline-none transition-all appearance-none bg-white">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-plyform-yellow/10 px-6 py-4 border-b border-gray-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-plyform-dark">Agent Settings</h2>
                ...
                <label class="flex items-start gap-4 p-4 border-2 border-gray-200 rounded-xl hover:border-plyform-yellow hover:bg-plyform-yellow/10 cursor-pointer transition-all group">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2">
                            <span class="font-semibold text-plyform-dark group-hover:text-plyform-dark">Feature this agent</span>
                            <span class="px-2 py-0.5 bg-plyform-yellow text-plyform-dark text-xs font-medium rounded-full">Recommended</span>
                        </div>
                        <p class="text-sm text-gray-600 mt-1">Display prominently on your agency website and listings</p>
                    </div>
                </label>

                <label class="flex items-start gap-4 p-4 border-2 border-gray-200 rounded-xl hover:border-plyform-mint hover:bg-plyform-mint/10 cursor-pointer transition-all group">
                    <input type="checkbox" 
                           name="is_accepting_new_listings" 
                           value="1"
                           {{ old('is_accepting_new_listings', true) ? 'checked' : '' }}
                           class="mt-1 w-5 h-5 text-plyform-mint border-gray-300 rounded focus:ring-plyform-mint/30">
                    <div class="flex-1">
                        <div class="font-semibold text-plyform-dark group-hover:text-plyform-dark">Accepting new listings</div>
                        <p class="text-sm text-gray-600 mt-1">Agent is available to take on new properties and clients</p>
                    </div>
                </label>

                <label class="flex items-start gap-4 p-4 border-2 border-gray-200 rounded-xl hover:border-plyform-purple hover:bg-plyform-purple/5 cursor-pointer transition-all group">
                    <input type="checkbox" 
                           name="send_invitation" 
                           value="1"
                           {{ old('send_invitation', true) ? 'checked' : '' }}
                           class="mt-1 w-5 h-5 text-plyform-purple border-gray-300 rounded focus:ring-plyform-purple/20">
                    <div class="flex-1">
                        <div class="font-semibold text-plyform-dark group-hover:text-plyform-dark">Send account invitation</div>
                        <p class="text-sm text-gray-600 mt-1">Agent will receive an email to set their password and access their account</p>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div class="sticky bottom-0 bg-white border-t border-gray-200 shadow-lg rounded-t-2xl p-6 -mx-4 md:mx-0 md:rounded-2xl">
            <div class="flex flex-col sm:flex-row items-center gap-4">
                <button type="submit" 
                        class="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-plyform-yellow to-plyform-mint hover:from-plyform-yellow/90 hover:to-plyform-mint/90 text-plyform-dark font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
