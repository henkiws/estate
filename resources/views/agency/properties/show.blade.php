    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <h1 class="text-3xl font-bold text-gray-900">{{ $property->full_address }}</h1>
                ...
                @elseif($property->status == 'sold')
                    <span class="px-3 py-1 bg-blue-100 text-blue-800 text-sm font-semibold rounded-full">Sold</span>
                @elseif($property->status == 'leased')
                    <span class="px-3 py-1 bg-purple-100 text-purple-800 text-sm font-semibold rounded-full">Leased</span>
                @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="border-t border-gray-200 pt-6 mt-6">
                        ...
                        <a href="{{ Storage::url($property->floorplan_path) }}" target="_blank" class="inline-block">
                            @if(str_ends_with($property->floorplan_path, '.pdf'))
                                <div class="flex items-center gap-3 p-4 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition">
                                    <svg class="w-12 h-12 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                                    </svg>
                                    <div>
                                        <div class="font-medium text-gray-900">View Floorplan PDF</div>
                                        <div class="text-sm text-gray-600">Click to open</div>
                                    </div>
                                </div>
                            @else
                                <img src="{{ Storage::url($property->floorplan_path) }}" alt="Floorplan" class="max-w-full h-auto rounded-lg border border-gray-200">
                            @endif
                        </a>
                    </div>
                @endif
            ...
            @if($property->agents->count() > 0)
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h2 class="text-2xl font-bold text-gray-900 mb-6">Assigned Agents</h2>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-2">
                    <a href="{{ route('agency.properties.edit', $property) }}" 
                       class="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-center font-medium flex items-center justify-center gap-2">
                    ...
                    @if($property->status == 'draft')
                        <form action="{{ route('agency.properties.publish', $property) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition text-center font-medium flex items-center justify-center gap-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @elseif($property->status == 'active')
                        <form action="{{ route('agency.properties.unpublish', $property) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition text-center font-medium flex items-center justify-center gap-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form action="{{ route('agency.properties.toggle-featured', $property) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition text-center font-medium flex items-center justify-center gap-2">
                    ...
                    <form action="{{ route('agency.properties.destroy', $property) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this property?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition text-center font-medium flex items-center justify-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Public Link -->
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 class="font-bold text-gray-900 mb-4">Public Listing Link</h3>
                <div class="space-y-3">
                    ...
                    <button onclick="copyPublicUrl()" class="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path>
                        </svg>
                        <span id="copyBtnText">Copy Link</span>
                    </button>
                    <a href="{{ $property->public_url }}" target="_blank" class="w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-semibold text-gray-900">{{ number_format($property->view_count) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Applications</span>
                        <span class="font-semibold text-gray-900">{{ $property->applications->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Images</span>
                        <span class="font-semibold text-gray-900">{{ $property->images->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
            ...
            <!-- Property Info -->
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 class="font-bold text-gray-900 mb-4">Property Information</h3>
                <div class="space-y-3 text-sm">
                    <div>
                        <span class="text-gray-600">Property Type:</span>
                        <span class="font-medium text-gray-900 ml-2">{{ ucfirst($property->property_type) }}</span>
                    </div>
                    <div>
                        <span class="text-gray-600">Listing Type:</span>
                        <span class="font-medium text-gray-900 ml-2">For {{ ucfirst($property->listing_type) }}</span>
                    </div>
                    @if($property->available_from)
                        <div>
                            <span class="text-gray-600">Available From:</span>
                            <span class="font-medium text-gray-900 ml-2">{{ $property->available_from->format('d M Y') }}</span>
                        </div>
                    @endif
                </div>
            </div>
