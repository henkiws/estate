...
<div class="py-8">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="p-6">
                ...
                <div class="flex items-start justify-between gap-4 mb-4">
                    ...
                    @if($ticket->status !== 'closed')
                        <form method="POST" action="{{ route('agency.support.close', $ticket) }}" onsubmit="return confirm('Are you sure you want to close this ticket?');">
                            @csrf
                            @method('PATCH')
                            <button 
                                type="submit" 
                ...
                <div class="flex items-start gap-4">
                    <div class="flex-shrink-0">
                        <div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                            <span class="text-blue-600 font-bold text-sm">{{ substr($ticket->user->name, 0, 1) }}</span>
                        </div>
                    </div>
                    <div class="flex-1">
        ...
        @foreach($ticket->replies as $reply)
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-4 {{ $reply->is_staff_reply ? 'ml-8 border-l-4 border-l-blue-500' : '' }}">
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0">
                            <div class="w-10 h-10 {{ $reply->is_staff_reply ? 'bg-blue-500' : 'bg-gray-200' }} rounded-full flex items-center justify-center">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="font-semibold text-gray-900">
                                    {{ $reply->user->name }}
                                    @if($reply->is_staff_reply)
                                        <span class="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-semibold rounded">Support Team</span>
                                    @endif
                                </span>
        ...
        @if($ticket->status !== 'closed')
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('agency.support.reply', $ticket) }}" enctype="multipart/form-data">
                        @csrf
