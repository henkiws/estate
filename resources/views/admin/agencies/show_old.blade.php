...
<div class="mb-8">
    <div class="flex items-start gap-4 mb-4">
        ...
        <div class="flex-1">
            <h1 class="text-3xl font-bold text-gray-800 mb-1">{{ $agency->agency_name }}</h1>
...
<div class="grid lg:grid-cols-3 gap-8">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex gap-2 ml-4">
                                        ...
                                        <a href="{{ route('admin.agencies.documents.preview', [$agency->id, $doc->id]) }}" 
                                           target="_blank"
                                           class="p-2.5 bg-[#E6FF4B] text-gray-800 rounded-lg hover:bg-[#E6FF4B]/80 transition-colors shadow-md hover:shadow-lg"
                            ...
                            <div class="flex gap-3">
                                <form action="{{ route('admin.agencies.documents.approve', [$agency->id, $doc->id]) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" 
                                            ...
                                            class="px-5 py-2.5 bg-[#DDEECD] text-gray-800 text-sm font-bold rounded-lg hover:bg-[#DDEECD]/80 transition-all shadow-md hover:shadow-lg flex items-center gap-2">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button onclick="openRejectDocModal({{ $doc->id }}, '{{ $doc->name }}')" 
                                        class="px-5 py-2.5 bg-gray-600 text-white text-sm font-bold rounded-lg hover:bg-gray-700 transition-all shadow-md hover:shadow-lg flex items-center gap-2">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                    </svg>
                                    Reject Document
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-md
                                ...
                                {{ str_contains(strtolower($log->description), 'suspended') ? 'bg-gray-300' : '' }}
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center gap-4 p-4 border border-gray-200 rounded-xl hover:border-[#DDEECD] hover:shadow-md transition-all">
                        <div class="w-12 h-12 bg-gradient-to-br from-[#DDEECD] to-[#4ADE80] rounded-full flex items-center justify-center text-gray-700 font-bold text-lg shadow-md">
                            {{ strtoupper(substr($agent->agent_name, 0, 2)) }}
                        </div>
                        <div class="flex-1">
                            <h3 class="font-bold text-gray-800">{{ $agent->agent_name }}</h3>
                            <p class="text-sm text-gray-600">{{ $agent->position ?? 'Sales Agent' }}</p>
                        </div>
                        <span class="px-3 py-1 bg-[#DDEECD] text-gray-700 rounded-full text-xs font-bold">
    ...
    <div class="space-y-6">
        ...
        {{-- Subscription Info --}}
        @if($subscription)
        <div class="bg-gradient-to-br from-[#E6FF4B]/50 to-[#DDEECD]/50 rounded-2xl p-6 text-gray-800 shadow-xl border-2 border-[#E6FF4B]">
            ...
            <div class="space-y-3">
                <div>
                    <p class="text-gray-700 text-sm font-medium">Plan</p>
                ...
                <div class="h-px bg-gray-700/20"></div>
                <div>
                    <p class="text-gray-700 text-sm font-medium">Next Billing</p>
...
<div id="rejectModal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-2xl w-full p-6 animate-slideUp shadow-2xl">
        ...
        <form action="{{ route('admin.agencies.reject', $agency->id) }}" method="POST">
            ...
            <div class="bg-[#E6FF4B]/30 border-l-4 border-[#E6FF4B] rounded-lg p-4 mb-4">
                <div class="flex">
                    <svg class="w-5 h-5 text-gray-700 mr-2 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" class="flex-1 px-4 py-3 bg-gray-600 text-white rounded-xl font-bold hover:bg-gray-700 transition-colors shadow-lg hover:shadow-xl">
                    Reject Agency
                </button>
            </div>
        </form>
